      ...
      for (const auto& track : media_stream->GetAudioTracks()) {
        ...
        if (!configuration_.audio.empty() && transceiver.ok()) {
          ...
          for (const auto& audio : configuration_.audio) {
            webrtc::RtpCodecCapability capabilitity;
            capabilitity.name = audio_codec_names.at(audio.codec.name);
            if (audio.codec.channel_count != 0) {
              capabilitity.num_channels = audio.codec.channel_count;
            }
            codecs.push_back(capabilitity);
          }
      ...
      for (const auto& track : media_stream->GetVideoTracks()) {
        ...
        if (!configuration_.video.empty() && transceiver.ok()) {
          ...
          for (const auto& video : configuration_.video) {
            webrtc::RtpCodecCapability capabilitity;
            capabilitity.name = video_codec_names.at(video.codec.name);
            capabilitity.kind = cricket::MEDIA_TYPE_VIDEO;
            // VideoCodecParameters.profile is documented as ignored. But in
            // case some application needs specifing profile-level-id, it still
            // passes to RtpCodecCapability. This API will be changed in the
            // future to align with WebRTC APIs.
            if (!video.codec.profile.empty()) {
              capabilitity.parameters["profile-level-id"] = video.codec.profile;
            }
            codecs.push_back(capabilitity);
          }
          transceiver.value()->SetCodecPreferences(codecs);
